...
from database import db, now, log_action
...
class ModUtils(commands.Cog):
    ...
    async def transfer(self, interaction: discord.Interaction, member: discord.Member, channel: discord.TextChannel):
        ...
        await notify_and_log(
            ...
            case_id=row["case_id"],
            detail=f"Transferred to {channel.name}",
            dm_target=member,
            dm_title="You Have Been Transferred to a New Cell",
            dm_fields=[
                ("Case ID", f"#{row['case_id']}", True),
                ("New Cell", channel.mention, True),
                ("Previous Cell", old_channel.mention if old_channel else "Unknown", True),
            ],
            log_title="Member Transferred",
            log_fields=[
    ...
    async def notify(self, interaction: discord.Interaction, member: discord.Member):
        ...
        await log_action(interaction.guild.id, "notify_resend", user_id=member.id,
                          moderator_id=interaction.user.id, case_id=row["case_id"], detail="Resent jail notification")
