...
class Cell(commands.Cog):
    ...
    async def lock(self, interaction: discord.Interaction, member: discord.Member):
        ...
        await interaction.followup.send(embed=build_embed("Jail Cell Locked", f"{member.mention} can no longer send messages in {channel.mention}."))
    ...
    async def slowmode(self, interaction: discord.Interaction, member: discord.Member, seconds: app_commands.Range[int, 0, 21600]):
        ...
        await interaction.followup.send(embed=build_embed("Slowmode Updated", f"Slowmode in {channel.mention} set to {seconds} second(s)."))
