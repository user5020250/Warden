...
class AppealDecisionView(discord.ui.View):
    ...
    async def _decide(self, interaction: discord.Interaction, decision: str):
        ...
        await log_action(interaction.guild.id, f"appeal_{decision}", user_id=appeal["user_id"],
                          moderator_id=interaction.user.id, case_id=appeal["case_id"], detail=result_text)
        ...
        if member is not None and cfg["dm_notifications"]:
            try:
                await member.send(embed=build_embed(
                    f"Your Appeal Was {decision_label}",
                    None,
                    fields=[
                        ("Case ID", f"#{appeal['case_id']}", True),
...
class Appeals(commands.Cog):
    ...
    async def submit(self, interaction: discord.Interaction, message: str):
        ...
        await log_action(interaction.guild.id, "appeal_submitted", user_id=interaction.user.id,
                          case_id=case["case_id"], detail=message)
    ...
    async def withdraw(self, interaction: discord.Interaction):
        ...
        await log_action(interaction.guild.id, "appeal_withdrawn", user_id=interaction.user.id,
                          case_id=row["case_id"])
